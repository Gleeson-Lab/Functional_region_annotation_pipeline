...
def summarize_annotations(tumor_id, exon_vf, vf, gnomad_dropped, repeats, \
                          homopolymer, tumor_ci, normal_ci):
    ...
    content = np.array([
                   ...
                   tumor_ref_count, tumor_alt_count, tumor_maf, tumor_lower_ci,
                   tumor_upper_ci, tumor_is_greater, \
                   normal_ref_count, normal_alt_count, normal_maf,
                    normal_lower_ci, normal_upper_ci, normal_is_greater, \
                   is_blood, is_sperm])
    
    content_dict = {}
    for i in range(len(HEADER)):
        content_dict[HEADER[i]] = content[i]
        #print('the length of ' + HEADER[i] + ' is ' +str(len(content[i])))

    #print(content_dict)
    
    #05/30/22: np.transpose() no longer transform the (29,) array to 29-column
    #array. Instead, try create a content_dict to associate each item in HEADER
    #with its corresponding column and generate the df afterwards

    final_df = pd.DataFrame(data = content_dict)
    return final_df 
...
def main(argv):
    if len(argv) != 11:
        sys.stderr.write("usage: " + argv[0] + "<tumor_id> <exon_vf> <vf> <gnomad_dropped> <repeats> \
                                                <homopolymer> <tumor_ci> <normal_ci> <func_region_anno_vcf> <output> \n")
        sys.exit(2)
    ...
    tumor_ci = argv[7]
    normal_ci = argv[8]
    func_region_anno_vcf = argv[9]
    outfile = argv[10]

    anno_df = summarize_annotations(tumor_id, exon_vf, vf, gnomad_dropped, repeats,\
                          homopolymer, tumor_ci, normal_ci)
    
    #combine the functional and region annotation results to the annotated variants table
    func_region_anno_df = pd.read_csv(func_region_anno_vcf, sep="\t")
    func_region_anno_df = func_region_anno_df.drop(columns=["End"])
    func_region_anno_df = func_region_anno_df.rename(columns={"Chrom": "CHROM", "Start":"POS","Ref":"REF", "Alt":"ALT"})
    anno_all_df = anno_df.merge(func_region_anno_df, on=["CHROM","POS","REF","ALT"], how="left")

    anno_all_df.to_csv(outfile,sep ="\t", index=False, header=True)
